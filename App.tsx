...
import { LogOut, QrCode, Home, GraduationCap, ChevronRight, Plus, X, LayoutDashboard, Calendar, Bell, User as UserIcon, Check, Trash2, FileText } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'assignments' | 'forum'>('assignments');
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    { id: 1, title: 'Selamat Datang', message: 'Selamat datang di semester baru!', time: 'Baru saja', read: false, type: 'info' }
  ]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row font-sans">
      ...
      {/* Modern Dark Sidebar */}
      <aside className="w-full md:w-72 bg-slate-900 text-white md:h-screen md:sticky md:top-0 z-20 flex flex-col shadow-xl">
        <div className="p-6 border-b border-slate-800">
          ...
          <p className="text-xs text-slate-400 mt-2 ml-1">Sistem Akademik Terpadu</p>
        </div>

        <div className="p-4 flex flex-col gap-2 flex-1">
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 mb-2 mt-2">Menu Utama</div>
          ...
          <div className="mt-6 text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 mb-2">Mata Kuliah Anda</div>
          {courses.slice(0, 3).map(course => (
             <button
                key={course.id}
                onClick={() => setSelectedCourse(course)}
                className={`w-full text-left px-4 py-2.5 rounded-lg flex items-center gap-3 text-sm transition-colors ${
                    selectedCourse?.id === course.id ? 'bg-slate-800 text-primary-400' : 'text-slate-400 hover:text-white'
                }`}
             >
                <div className={`w-2 h-2 rounded-full ${
                    course.code.startsWith('SIL') ? 'bg-emerald-500' : 
                    course.code.startsWith('MM') ? 'bg-blue-500' : 'bg-purple-500'
                }`}></div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
           <div className="flex items-center gap-3 mb-4 px-2">
              <img src={user.avatarUrl} className="w-10 h-10 rounded-full border-2 border-slate-700" alt="User" />
              <div className="overflow-hidden">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                     ...
                     <span className="truncate">{user.role === 'lecturer' ? 'Dosen' : 'Mahasiswa'}</span>
                  </div>
              </div>
           </div>
           <button 
             onClick={() => setUser(null)}
             className="w-full flex items-center justify-center gap-2 text-red-400 bg-red-500/10 hover:bg-red-500/20 py-2.5 rounded-lg text-sm font-medium transition-colors border border-red-500/20"
           >
      ...
      <main className="flex-1 overflow-y-auto relative">
        ...
        <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-200 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm text-gray-500">
                <span className="cursor-pointer hover:text-primary-600 font-medium" onClick={() => setSelectedCourse(null)}>Home</span>
                {selectedCourse && (
                    <>
                        <ChevronRight size={14} />
                        <span className="font-bold text-gray-900 truncate max-w-[150px] sm:max-w-none">{selectedCourse.name}</span>
                    </>
                )}
            </div>
            <div className="flex items-center gap-4">
                ...
                <div className="relative">
                    ...
                    {/* Dropdown Menu */}
                    {showNotifications && (
                        <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50 animate-in fade-in zoom-in duration-200 origin-top-right">
                                            ...
                                            <div className="flex gap-3">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1">
                                                        {notif.type === 'assignment' && <FileText size={12} className="text-primary-500" />}
                                                        <h4 className={`text-xs ${notif.read ? 'font-medium text-gray-600' : 'font-bold text-gray-800'}`}>{notif.title}</h4>
                                                    </div>
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto pb-24">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className={`rounded-3xl shadow-lg p-8 mb-8 text-white relative overflow-hidden ${
                     selectedCourse.code.startsWith('SIL') ? 'bg-gradient-to-r from-emerald-600 to-teal-500' : 
                     selectedCourse.code.startsWith('MM') ? 'bg-gradient-to-r from-blue-600 to-indigo-500' : 'bg-gradient-to-r from-purple-600 to-pink-500'
                }`}>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="flex items-center gap-6 mt-6 pt-6 border-t border-white/20">
                            <div className="flex items-center gap-2">
                                <UserIcon size={16} />
                                <span className="text-sm font-medium">Pengajar: {user?.role === 'lecturer' ? 'Anda' : 'Dr. Budi Santoso'}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Calendar size={16} />
                ...
                <div className="flex gap-6 border-b border-gray-200 mb-6">
                    <button 
                        ...
                        className={`pb-3 px-2 text-sm font-bold transition-all relative ${
                            activeTab === 'assignments' ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'
                        }`}
                    >
                        Tugas & Pengumpulan
                        {activeTab === 'assignments' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-primary-600 rounded-full"></div>}
                    </button>
                    <button 
                        onClick={() => setActiveTab('forum')}
                        className={`pb-3 px-2 text-sm font-bold transition-all relative ${
                            activeTab === 'forum' ? 'text-primary-600' : 'text-gray-500 hover:text-gray-700'
                        }`}
                    >
                        Forum Diskusi
                        {activeTab === 'forum' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-primary-600 rounded-full"></div>}
                    </button>
                </div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-gray-500 text-sm font-medium">Total Matakuliah</p>
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-gray-500 text-sm font-medium">Tugas Aktif</p>
                            <h3 className="text-2xl font-bold text-gray-800">{activeAssignmentsCount}</h3>
                        </div>
                    </div>
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-gray-500 text-sm font-medium">Deadline Dekat</p>
                            <h3 className="text-2xl font-bold text-gray-800">{upcomingDeadlines}</h3>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-xl font-bold text-gray-800">Matakuliah Terdaftar</h2>
                        {user.role === 'lecturer' && (
                            <button 
      ...
      {/* ID Card Modal */}
      <IDCard user={user} isOpen={showIDCard} onClose={() => setShowIDCard(false)} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl animate-in fade-in zoom-in duration-200">
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                    <h3 className="text-lg font-bold text-gray-800">Tambah Mata Kuliah</h3>
