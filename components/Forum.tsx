    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
            ...
            <p className="text-sm text-gray-500">Ruang tanya jawab dan diskusi kelas</p>
        </div>
        <button 
      ...
      {summary && (
        <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 p-6 rounded-2xl animate-in fade-in slide-in-from-top-4 shadow-sm relative overflow-hidden">
            ...
            <h3 className="font-bold text-purple-900 text-sm mb-3 flex items-center gap-2 relative z-10">
                <Sparkles size={16} className="text-purple-600" /> Ringkasan Diskusi (AI Generated)
            </h3>
          ...
          <div className="flex-1">
             <form onSubmit={handlePostSubmit}>
                ...
                <div className="flex justify-between items-center">
                    <p className="text-xs text-gray-400">Tekan enter untuk baris baru</p>
                    <button 
                        type="submit" 
      ...
      <div className="space-y-4">
        {posts.length === 0 ? (
            <div className="text-center py-12">
                ...
                <p className="text-gray-400 text-sm">Jadilah yang pertama memulai percakapan!</p>
            </div>
        ) : (
            posts.map(post => (
            <div key={post.id} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                        <img src={`https://ui-avatars.com/api/?name=${post.authorName}&background=random`} alt={post.authorName} className="w-10 h-10 rounded-full" />
                        <div>
                            <div className="flex items-center gap-2">
                                <h4 className="font-bold text-gray-900 text-sm">{post.authorName}</h4>
                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${
                                    post.authorRole === 'lecturer' ? 'bg-primary-100 text-primary-700' : 'bg-gray-100 text-gray-600'
                                }`}>
                                    {post.authorRole === 'lecturer' ? 'Dosen' : 'Mahasiswa'}
                                </span>
                            </div>
                            <span className="text-xs text-gray-400">{new Date(post.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', hour: '2-digit', minute:'2-digit' })}</span>
                        </div>
                    </div>
                ...
                <div className="ml-[52px] mt-3">
                     <button className="text-xs font-semibold text-primary-600 hover:underline">Balas</button>
                </div>
            </div>
            ))
        )}
